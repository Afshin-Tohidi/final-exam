...
const foodData = function(category) {
    fetch(`http://localhost:3131/foods/${category}`).then((response)=>response.json()).then((data1)=>{
        renderFoodCards(data1);
        console.log(data1);
    });
};
foodData();
const renderFoodCards = (data1)=>{
...
const updateQuantity = (productId, change)=>{
    const food = data.find((food)=>food.id == productId);
    if (food) {
        food.qty += change;
...
},{}]},["j2YDk","1SICI"], "1SICI", "parcelRequiree33f")
